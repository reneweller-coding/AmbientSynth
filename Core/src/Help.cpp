...
const HelpEntry kHelp[] = {
    ...
    { "srcN_env", "Give this source one of the preset's six envelope shapes as its own level contour, instead of the plain Delay-and-Rise entrance. The shape runs from the note, not from the phrase, so every note gets it; it is read with that envelope's own Mode and Time, and it is the same shape the modulation matrix uses, not a seventh envelope. The shapes are bipolar: -1 is silence, +1 is the source at its written Level. Off by default." },
    ...
    { "srcN_root", "Wavetable: a floor under the fundamental. A wavetable here is a spectrum, and a spectrum cut from a recording keeps what that recording had -- which for a bell, a bowed harmonic or overtone singing is very little at the bottom. Measured over 1200 frames of the library the energy sits at partial 5.8 on average, the fundamental holds a quarter of it, and a quarter of all frames have less than a tenth of their energy there. Played high that is the material speaking; played low it is why such a source sounds thin and seems to work only up top -- what is heard is the sixth partial of a note whose own pitch is not in the sound. Turned up, the fundamental is brought to half the frame's energy and the rest is pulled down to make room, so this moves weight about rather than adding level. At 0 the frame is exactly what was analysed." },
    ...
    { "partial_spread", "The partials of the additive bank spread across the stereo field one by one, each at its own place, the pattern turning once every fifty seconds (Madrona Labs' Sumu spreads its partials this way). The fundamental and its neighbours are not favoured: neighbours in the series sit far apart, by the golden angle, so nothing harmonically related clumps to one side. Equal power per partial, so the mono sum keeps its level. The whole voice is still placed by its distance and pan as before; this is width inside the note rather than around it. 0 is the bank as it always was, summed once." },
...
Plain text files (*.ambientpack, one preset per line) that may name a sample, a wavetable, an impulse response, a modulation matrix and envelope shapes of their own. Put them in Documents/AmbientSynth/Packs or point AMBIENT_PACKS at a folder (the installer's own folders are read too, and a pack found in two of them loads once); they appear everywhere the built-in presets do, each pack as a family. The library that ships alongside has 8400 presets in 42 packs, 6354 samples (2556 textures and 3798 field recordings), 2096 wavetables and 546 impulse responses.
